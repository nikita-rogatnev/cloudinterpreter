...
//            url: 'http://signaling.simplewebrtc.com:8888',
                url: 'https://cloudinterpreter.com:8888',
                log: false,
                localVideoEl: '',
                remoteVideosEl: '',
        ...
        connection = this.connection = io.connect(this.config.url, {secure: true});
        ...
        connection.on('joined', function (room) {
            ...
            var userinfo = {};
            userinfo.name = jQuery('#userinfo').html();

//            console.log('my userinfo',userinfo);

            connection.emit('set userinfo', userinfo);

            //console.log('some userinfo: ', userinfo);
        ...
        connection.on('UserInfoWasSet', function(){
            connection.emit('get userinfo');
        });

        connection.on('send userinfo', function(objects) {
            userinfo_storage = objects;
            console.log('userinfo storage: ',userinfo_storage);

            jQuery.each(userinfo_storage, function(i, item) {
                //console.log('i: ',i)
                //console.log('item: ',item)

                jQuery('.user-description-'+i).html(item);
            })
...
// that will be used to put the video tag into.
    ...
    WebRTC.prototype.startLocalVideo = function (element) {
        ...
        //console.log("config",this.config.media)
    ...
    Conversation.prototype.handleRemoteStreamAdded = function (event) {
        ...
        var user_id = this.id;
        var some_info_about_user = userinfo_storage[user_id];

        //console.log('some info about user came here: ', userinfo_storage);

        el.id = this.id;
        attachMediaStream(el, stream);

        el.style.width = '530px';
        // el.style.marginRight = '46px';

        //new element on the page
        jQuery('#videoLine').append('<div style="float:left; padding-top:0px;  width: 455px;" id="'+el.id+'-container" class="remote-video-container"></div>');
        jQuery('#'+el.id+'-container').append(el);

        var i=1;
        jQuery('.remote-video-container').each(function() {
            //alert(jQuery(this).css('width'))
            if(i==1) {
                jQuery(this).css('margin-right',102+'px');
            }
            if(i == 2) {
                i=0;
            }
        ...
        add_video_control(el.id);
        add_user_info(el.id,some_info_about_user);
...
//    el.volume = 0.5;
    ...
    Conversation.prototype.handleStreamRemoved = function () {
        ...
        jQuery('#'+this.id+'-container').remove();
